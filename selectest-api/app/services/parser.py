...
async def parse_and_store(session: AsyncSession) -> int:
    ...
    try:
        # FIX (баг №4): контекстный менеджер для автоматического закрытия клиента
                    ...
                    parsed_payloads.append(
                        {
                            ...
                            # FIX (баг №3): безопасное получение города
        ...
        logger.exception("Непредвиденная ошибка парсинга: %s", exc)
        return 0
